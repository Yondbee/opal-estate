...
class OpalEstate_Search{
	...
	public static function get_search_results_query( $limit = 9 ){
		...
		$tax_query = array();
		if( isset( $_GET['location']) &&  $_GET['location'][0] !=-1 ){
		    if($_GET['location'][1] != '-1') {
                unset($_GET['location'][0]);
		        $location = $_GET['location'];
            } else if($_GET['location'][1] == '-1') {
		...
		if( isset( $_GET['types']) && $_GET['types'] !=-1 ){
            if( isset( $_GET['subtype']) && $_GET['subtype'] !=-1 ){
                $tax_query[] =
                  array(
                    'taxonomy' => 'opalestate_types',
                    'field'    => 'slug',
        ...
        if(!empty($s)) {
            array_push($args['meta_query'], array(
              'key'     => OPALESTATE_PROPERTY_PREFIX.'sku',
              'value'   => $s,
              'compare' => 'LIKE',
              'type' => 'text'
            ));
